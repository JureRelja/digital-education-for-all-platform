...
function Main() {
  ...
  const [currentPath, setCurrentPath] = useState(""); // Current page path
  ...
  useEffect(() => {
    switch (location.pathname) {
      ...
      case "/dashboard/job-suggestions":
        setCurrentPath("Job suggestions");
        break;
    }
  }, [location.pathname]);
  ...
  return (
    <div className="py-2">
      ...
      <div className="rounded-[30px] min-w-0 flex-1 flex flex-col justify-between items-center pb-3 bg-slate-100 px-4 md:px-[22px] max-w-full md:max-w-auto before:content-[''] before:w-full before:h-px before:block min-h-[80vh]">
        ...
        <div className="mt-5 px-5 py-2 xl:border-t-slate-300 xl:border-t-[1px] flex flex-col gap-2 text-center justify-center items-center align-middle w-fit">
          <img src={bannersUrl} className="w-2/4" />
